...
int main(int argc, char* argv[])
{
  ...
  //
  // where is input coming from, keyboard or sql program file?
  //
  ...
  //
  // input the tokens, either from keyboard or the given sql file; the
  // "input" variable controls the source. We use the scanner to input
  // the tokens one by one from the input stream. We'll do this over
  // and over again for testing purposes, until user enters $:
  //
  while (true)
  {
    int lineNumber = -1;
    ...
    else // consume what's left of the input stream before we prompt again:
    {
